...
export default function ConvertPrerequisiteToSupplyDialog({
  children,
  product,
  data,
  setData,
  ...
  const { convertPrerequisiteToSupply } = useSupply();
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    showLoading();
    try {
      const { quantity } = e.target as typeof e.target & IConvertToSupplyForm;
      const response = await convertPrerequisiteToSupply(
        product.productId,
        quantity.value
      );
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px] bg-zinc-950 text-white">
        <DialogHeader>
          <DialogTitle>Convert Prerequisite to Supply</DialogTitle>
          <DialogDescription>
